...
interface ItemDao2 {
    ...
    suspend fun insert2(item2: Item2) // 함수를 정지함수로 만들어서 코루틴에서 실행할 수 있도록 함

    // ** 수정 : 항목의 다른 속성 일부나 전부를 업데이트할 수 있음
    @Update
    suspend fun update2(item2: Item2)

    // ** 삭제 : 한 항목이나 항목 목록을 삭제
    @Delete
    suspend fun delete2(item2: Item2)

    // 나머지 기능에는 편의 주석이 없으므로 @Query 주석을 사용하여 SQLite 쿼리를 제공해야 함

    // ** 검색 : 주어진 id에 기반하여 항목 테이블에서 특정 항목을 검색
    @Query("SELECT * from itemTBL2 WHERE id2 = :id")
    fun getItem2(id: Int): Flow<Item2>

    // ** kotlinx.coroutines.flow.Flow에서 Flow를 가져오는 함수
    @Query("SELECT * from itemTBL2 ORDER BY routine2 ASC")
    fun getItems2(): Flow<List<Item2>>
}